...
class TransE(MateModel):
    ...
    def train_epoch(self,
        ...
        """
        通过调用 self.train() 方法，你可以将模型切换到训练模式。
        默认情况下，PyTorch的模型在创建时是处于训练模式的。
        在训练模式下，模型会跟踪参数的梯度，允许你进行反向传播和权重更新。
        要将模型切换到评估/测试模式，可以使用 self.eval() 方法。
        在评估模式下，模型不会跟踪参数的梯度，这有助于加速推断和减少内存消耗。
        """
        self.train()
        self.opt.zero_grad()
        # dps是数据处理函数
